...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$csrf_valido) {
        ...
        if (!empty($email) && !empty($password)) {
            ...
            if ($regIp) {
                // Janela de esquecimento do IP: se já passou o tempo de bloqueio, zera
                // (independente de quantas tentativas acumulou — o que importa aqui é só
                // se o prazo de espera já passou)
            ...
            if (!$ip_bloqueado) {
                ...
                if ($usuarioEncontrado) {
                    ...
                    // Reseta sempre que o prazo de bloqueio já passou, independente
                    // de quantas tentativas foram acumuladas.
                    // =========================================================
                    if ($usuario['tentativas_falhas'] > 0 && !empty($usuario['bloqueado_ate'])) {
                        $validade_erro = new DateTime($usuario['bloqueado_ate']);

                        if ($agora > $validade_erro) {
...
function registrarLogAcesso($db, $usuarioId, $email, $ip, $sucesso, $motivoFalha) {
    try {
        // O log_acessos é um registro de AUDITORIA, não de bloqueio — por isso o IP é
        // ofuscado aqui (LGPD - minimização de dados). O IP completo continua sendo usado
        // normalmente em tentativas_ip, que é a tabela que realmente decide o bloqueio.
        $ipOfuscado = obfuscateIp($ip);
