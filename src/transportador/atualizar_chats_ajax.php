<?php
// src/transportador/atualizar_chats_ajax.php
session_start();
...
try {
    ...
    // Ajustar consulta baseada no tipo de usuário
    $sql_novas = "";
    $params = [];
    
    if ($usuario_tipo === 'transportador') {
        // Para transportador: verificar conversas onde ele é o transportador
        $sql_novas = "SELECT 
                        cm.conversa_id,
                      ...
                      AND cc.transportador_excluiu = 0
                      GROUP BY cm.conversa_id";
    } else {
        // Para comprador/vendedor (código original)
        $sql_novas = "SELECT 
                        cm.conversa_id,
                        COUNT(cm.id) as total_novas,
                        MAX(cm.data_envio) as ultima_data
    ...
    if ($usuario_tipo === 'transportador') {
        ...
        if ($mostrar_arquivados) {
            $sql_total_nao_lidas .= " AND cc.favorito_transportador = 1";
        } else {
            $sql_total_nao_lidas .= " AND cc.favorito_transportador = 0";
        }
    } else {
        $sql_total_nao_lidas = "SELECT 
                                cc.id as conversa_id,
